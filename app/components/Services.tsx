...
function PlanModal({ open, onClose, subject }: { open: boolean, onClose: () => void, subject: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-black p-6 sm:p-8 rounded-2xl border-2 border-[#B5FF6D] shadow-2xl relative max-w-md w-full" style={{ boxShadow: '0 0 32px 8px #B5FF6D88' }}>
        <button onClick={onClose} className="absolute top-4 right-4 text-[#B5FF6D] text-2xl font-bold">&times;</button>
        <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">Get Started with {subject}</h2>
        {state.succeeded ? (
          <p className="text-[#B5FF6D] text-lg font-semibold">Thanks for your interest! We'll contact you soon.</p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-[#B5FF6D] font-semibold mb-1 text-sm">Name</label>
              <input id="name" type="text" name="name" required className="w-full px-4 py-2 rounded-lg bg-gray-900 border border-[#B5FF6D]/40 text-white focus:border-[#B5FF6D] outline-none text-sm" />
            </div>
            <div>
              <label htmlFor="phone" className="block text-[#B5FF6D] font-semibold mb-1 text-sm">Phone Number</label>
              <input id="phone" type="tel" name="phone" required className="w-full px-4 py-2 rounded-lg bg-gray-900 border border-[#B5FF6D]/40 text-white focus:border-[#B5FF6D] outline-none text-sm" />
            </div>
            <div>
              <label htmlFor="email" className="block text-[#B5FF6D] font-semibold mb-1 text-sm">Email</label>
              <input id="email" type="email" name="email" required className="w-full px-4 py-2 rounded-lg bg-gray-900 border border-[#B5FF6D]/40 text-white focus:border-[#B5FF6D] outline-none text-sm" />
              <ValidationError prefix="Email" field="email" errors={state.errors} />
            </div>
            <div>
              <label htmlFor="subject" className="block text-[#B5FF6D] font-semibold mb-1 text-sm">Subject</label>
              <input id="subject" type="text" name="subject" value={subject} readOnly className="w-full px-4 py-2 rounded-lg bg-gray-900 border border-[#B5FF6D]/40 text-white focus:border-[#B5FF6D] outline-none text-sm" />
            </div>
            <div>
              <label htmlFor="message" className="block text-[#B5FF6D] font-semibold mb-1 text-sm">Message</label>
              <textarea id="message" name="message" rows={3} className="w-full px-4 py-2 rounded-lg bg-gray-900 border border-[#B5FF6D]/40 text-white focus:border-[#B5FF6D] outline-none text-sm" placeholder="Anything else you'd like to share?" />
              <ValidationError prefix="Message" field="message" errors={state.errors} />
            </div>
            <button type="submit" disabled={state.submitting} className="w-full py-3 bg-[#B5FF6D] text-black font-bold rounded-lg hover:bg-[#A3E85C] transition-all duration-300 mt-2 text-sm">Submit</button>
          </form>
        )}
      </div>
    </div>
        ...
        <section className="py-12 sm:py-20 bg-transparent relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center mb-4">
              <div className="inline-flex items-center space-x-2 px-4 sm:px-6 py-2 sm:py-3 bg-[#B5FF6D]/10 border border-[#B5FF6D]/30 rounded-full mb-2">
                <Star className="w-4 h-4 sm:w-5 sm:h-5 text-[#B5FF6D]" />
                <span className="text-[#B5FF6D] font-semibold text-sm sm:text-base">Services & Packages</span>
              </div>
              
              <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-2">Our Services & Packages</h1>
            ...
            {/* Service Navigation */}
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-0">
              {serviceKeys.map((key) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`flex items-center space-x-2 sm:space-x-3 px-3 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold transition-all duration-300 text-sm sm:text-base ${
                      activeService === key
        ...
        <section className="py-12 sm:py-20 bg-transparent relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center mb-2">
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2">{services[activeService].title}</h2>
              <p className="text-base sm:text-lg text-[#8A9A5B]">{services[activeService].description}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-4">
              {services[activeService].packages.map((pkg, index) => (
                ...
                >
                  <div
                    className={`relative bg-gray-800/50 p-4 sm:p-6 lg:p-8 rounded-xl border border-gray-700 shadow-2xl flex flex-col justify-between items-center text-center transition-all duration-500 hover:border-[#B5FF6D] hover:shadow-[#B5FF6D]/40`}
                    style={{ minHeight: '380px', maxWidth: '320px', aspectRatio: '1 / 1', overflow: 'visible', perspective: '800px' }}
                    onMouseMove={e => {
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center justify-center absolute inset-0 z-20 transition-all duration-500 group-hover:-translate-y-12 sm:group-hover:-translate-y-16 group-hover:scale-90 group-hover:opacity-0">
                      <img src="https://i.postimg.cc/DwwNNDsr/Whats-App-Image-2025-06-26-at-17-33-59-a813e2b2.jpg" alt="Weblancer Logo" className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl shadow-lg border-4 border-[#B5FF6D] bg-white object-contain" />
                      <span className="mt-3 sm:mt-4 text-base sm:text-lg font-bold text-[#B5FF6D] drop-shadow-[0_0_8px_#B5FF6D] animate-pulse">{pkg.name}</span>
                    </div>
                    ...
                    <div className="opacity-0 translate-y-8 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 delay-100 relative z-10 flex flex-col h-full w-full items-center justify-center">
                      {pkg.popular && (
                        <div className="absolute -top-4 sm:-top-6 left-1/2 transform -translate-x-1/2 z-30 flex justify-center w-full pointer-events-none">
                          <span className="px-3 sm:px-4 py-1 bg-[#B5FF6D] text-black text-xs font-bold rounded-full shadow-md border border-[#B5FF6D]/40 pointer-events-auto animate-pulse" style={{ minWidth: '110px', textAlign: 'center' }}>
                            Most Popular
                          </span>
                        </div>
                      )}
                      ...
                      <div className="mb-2 mt-8 sm:mt-10">
                        <div className="text-xl sm:text-2xl font-bold text-[#B5FF6D] mb-1">{pkg.price}</div>
                        <p className="text-[#8A9A5B] text-xs">{pkg.description}</p>
                      </div>

                      {/* Features list */}
                      <div className="space-y-1 sm:space-y-2 mb-3 sm:mb-4 flex-1 w-full px-1">
                        {pkg.features.map((feature, featureIndex) => (
                          <div key={featureIndex} className="flex items-center space-x-2">
                            <Check className="w-3 h-3 sm:w-4 sm:h-4 text-[#B5FF6D] flex-shrink-0" />
        ...
        <section className="py-12 sm:py-20 bg-transparent relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
            <div className="text-center mb-12 sm:mb-16">
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 sm:mb-6">Why Choose Weblancer?</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              <div className="group bg-gray-800/50 p-6 sm:p-8 rounded-xl border border-gray-700 hover:border-[#B5FF6D] transition-colors duration-300 relative shadow-lg hover:shadow-[0_0_24px_4px_#B5FF6D55] flex flex-col items-center">
                <Zap className="w-10 h-10 sm:w-12 sm:h-12 text-[#B5FF6D] mb-4 sm:mb-6 group-hover:animate-pulse" />
              ...
              <div className="group bg-gray-800/50 p-6 sm:p-8 rounded-xl border border-gray-700 hover:border-[#B5FF6D] transition-colors duration-300 relative shadow-lg hover:shadow-[0_0_24px_4px_#B5FF6D55] flex flex-col items-center">
                <Shield className="w-10 h-10 sm:w-12 sm:h-12 text-[#B5FF6D] mb-4 sm:mb-6 group-hover:animate-pulse" />
                <h3 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4">Quality Assurance</h3>
                <p className="text-[#8A9A5B] leading-relaxed text-sm sm:text-base">
              ...
              <div className="group bg-gray-800/50 p-6 sm:p-8 rounded-xl border border-gray-700 hover:border-[#B5FF6D] transition-colors duration-300 relative shadow-lg hover:shadow-[0_0_24px_4px_#B5FF6D55] flex flex-col items-center">
                <Globe className="w-10 h-10 sm:w-12 sm:h-12 text-[#B5FF6D] mb-4 sm:mb-6 group-hover:animate-pulse" />
